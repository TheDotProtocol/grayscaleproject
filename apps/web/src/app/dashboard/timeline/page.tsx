...
const CATEGORY_COLORS: Record<string, string> = {
  council: "border-primary/40 bg-primary/5",
  executive_discovery: "border-primary/40 bg-primary/5",
      ...
      <div className="relative space-y-0">
        {(timeline.data ?? []).map((entry, idx) => (
          <div key={entry.id} className="relative flex gap-4 pb-8">
            ...
            <div className="relative z-10 mt-1.5 h-3.5 w-3.5 shrink-0 rounded-full border-2 border-primary bg-[#0A0A0F]" />
            <div className={cn("flex-1 rounded-xl border p-4", CATEGORY_COLORS[entry.category] ?? "border-white/10 bg-white/[0.02]")}>
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="text-xs uppercase tracking-wider text-muted-foreground">{entry.category.replace(/_/g, " ")}</p>
                  <p className="font-medium text-white">{entry.title}</p>
                  {entry.summary && <p className="mt-1 text-sm text-muted-foreground">{entry.summary}</p>}
                </div>
                <time className="text-xs text-muted-foreground/70">
                  {new Date(entry.occurredAt).toLocaleString()}
                </time>
              </div>
              <p className="mt-2 text-xs text-muted-foreground/70">Source: {entry.source}</p>
            </div>
          </div>
        ))}
